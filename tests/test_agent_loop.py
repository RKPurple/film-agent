...
from cinemagent.agent import DUPLICATE_CALL_ERROR, FALLBACK_MESSAGE, collect, run_agent, with_trace_log
...
def scenario_cap():
    return dict(script=[tool_response(f"r{i}", ("filter_by_actor", {"name": f"Actor {i}"})) for i in range(1, 4)],
                max_iterations=3)
...
ALL_SCENARIOS = [scenario_happy, scenario_parallel, scenario_duplicate_within_response,
                 scenario_duplicate_across_iterations, scenario_cap, scenario_server_error,
...
def test_iteration_cap(fake_tools):
    ...
    assert done["iterations"] == 3 and done["model_calls"] == 3
    assert done["interaction_id"] == "r3"
